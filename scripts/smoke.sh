...
# Вывод забирается целиком и разбирается ЗДЕСЬ, а не фильтруется на площадке:
# фильтр удалённой стороны нечем подделать в тестах, и проверка была бы зелёной
# на любом ответе — поймано ровно так.
funnel=$(remote "tailscale funnel status 2>&1")
if printf '%s' "$funnel" | grep -q "127.0.0.1:8266"; then
