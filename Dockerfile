...
# Версия сборки (T246, #201). Внутри контейнера нет ни `.git`, ни репозитория,
# поэтому спросить git о версии некому — значение обязано попасть в образ при
# сборке. Аргумент стоит здесь, а не выше: он меняется на каждой сборке и выше
# обнулял бы кеш системных слоёв. Значения по умолчанию нет намеренно: образ,
# собранный мимо процедуры, обязан отвечать «неизвестна», а не выглядеть целым.
ARG BUILD_SHA=""
