...
def build_html(res, lang, photos, src=None):
    ...
    # или повторная, а письмо к документу не подшивают. Подпись `t["type"]`
    # лежала в словаре с самого начала и пустовала — печатать её было нечем
    # после снятия таблицы `TYPE_EN` (T177).
    #
    # Слово берётся тем же `inspection_kind`, что и письмо, а не собственным
    # обращением к таблице видов: два места, превращающих код в слово, — это два
    # места, которые разойдутся, и разойдутся молча. Оттуда же и правило языка:
    # `lang` здесь — язык СОБИРАЕМОГО документа, поэтому английский отчёт по
    # русской проверке называет вид по-английски, а не так, как его записали.
    for k, v in ((t["unit"], m.get("unit")), (t["city"], m.get("city")),
                 (t["partner"], m.get("partner")),
                 (t["auditor"], m.get("auditor")),
                 (t["type"], inspection_kind(m, lang)),
                 (t["date"], fmt_date(m.get("date")))):
        if v:
