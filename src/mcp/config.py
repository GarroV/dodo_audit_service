...
`docs/forge/blocks/db.md`). Снаружи арендатор не принимается ни в каком виде —
ни аргументом вызова, ни заголовком, ни по имени.

**Откуда он берётся — двух видов, и оба задаёт не спрашивающий** (T253):

1. Карта «арендатор=токен» из `.env` (`MCP_TOKENS`): токен принадлежит
   СТОРОНЕ, открывает всю её историю проверок, разбирается один раз на старте.
2. Личный токен, выпущенный ботом на конкретного человека (решение D098).
   Живёт отпечатком в базе (`src/db/mcp_access.py`), отзывается поимённо и
   немедленно, а арендатор у него записан при выпуске — из настройки стенда,
   не из разговора.

Проверяются они по очереди (`resolve_access`) и обе действуют одновременно.
Карту из `.env` не отключили намеренно: на момент перехода это единственный
живой доступ к серверу, и гасить его тем же движением, которым вводится
новый, значило бы устроить перерыв в работе ради опрятности.
...
def _tenant_from_store(token: str) -> str | None:
    ...
    (`src/db/mcp_access.py`); сверяются они здесь, потому что предъявляются
    сюда — блоки `bot` и `mcp` пиры и друг друга не импортируют, общее место у
    них одно, ярусом ниже.

    Импорт внутри функции по той же причине, что у инструментов
    (`src/mcp/tools.py`): `src.db.mcp_access` тянет `psycopg`, а жадный импорт
    наверху модуля потребовал бы его от каждого, кто читает настройки, — в том
    числе от проверок разбора `.env`, которым база не нужна вовсе.

    **Отказ базы не выдаётся за незнакомый токен молча.** Вернуть `None` тихо
    значило бы объяснить каждому владельцу личного токена его же словами
    «токен не опознан», пока настоящая причина — недоступная база; искать он
    пошёл бы свой токен, а чинить надо совсем другое. Поэтому причина уходит
    в журнал целиком, и уходит предупреждением, а не отладкой. Клиенту при
    этом по-прежнему достаётся общий отказ: перечислять по нему устройство
    сервера не должен никто.
    """
    try:
        from ..db.mcp_access import resolve_token
    except ImportError:
        # Драйвер базы не поставлен: стенд собран без блока `db`. Личных
        # токенов на нём не бывает, и это не отказ — карта из `.env` работает.
        return None
    try:
        владелец = resolve_token(token)
    except Exception:
        logger.warning(
            "личные токены не сверены — список доступа недоступен; "
            "проверка по карте %s при этом уже прошла и токен ею не опознан",
            MCP_TOKENS_VAR,
            exc_info=True,
        )
        return None
...
def resolve_access(settings: Settings, header: str | None) -> Access:
    ...
    **Дверей две, и порядок между ними важен** (T253). Сперва карта из `.env`
    — она в памяти и стоит одного сравнения; затем личные токены из базы,
    выпущенные ботом. Обратный порядок означал бы поход в базу на каждый
    запрос стороны, у которой токен и так в карте.

    Карта из `.env` продолжает работать намеренно. На момент перехода это
    ...
    **Право на снятие проверок личным токеном не выдаётся.** Оно спрашивается
    у той же карты (`may_retract`), где личного токена нет, — то есть остаётся
    там, где его назвали руками (`MCP_RETRACTION_TOKENS`). Это не упущение:
    снятие необратимо для партнёра, и раздавать его вместе с обычным доступом,
    выпускаемым в чате, — отдельное решение, которого никто не принимал.
