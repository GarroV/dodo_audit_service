...
#: он уезжает в базу к каждой посчитанной проверке.
#:
#: Белый список здесь законен, а у имени ВЕРСИИ — нет, и разница не в
#: строгости. Это единственное место, где имя набора придумывают ЗДЕСЬ И
#: СЕЙЧАС, — значит выбор ещё есть, и отказ с примером стоит человеку одного
#: слова. Имя уже изданного набора приходит от управляющей компании файлом
#: `checklist_version.txt` (`_resolve_name` наследует его как есть), бывает не
#: латиницей и выбором не является вовсе: отвергнутое, оно означало бы, что
#: продукт не читает методику своего заказчика (T243).
...
def _run(
    ...
    """Вердикт движка — или отказ окружения, если вердикта не будет.

    Ненулевой код возврата вердиктом считается НЕ всегда: движок объясняет
    словами каждый свой отказ, а умерший процесс, вышедший срок и не
    стартовавший интерпретатор ненулевым кодом тоже кончаются. Выданные за
    вердикт, они говорят вызывающему «движок вашу методику не принял» — и
    человек чинит методику, в которой чинить нечего (T187).

    Несостоявшийся старт при этом повторяется: до первой строки движка дело не
    дошло, кандидат не тронут. Об этом говорит сам интерпретатор своим
    `Python runtime state`, а не догадка по коду возврата.
    """
    for попытка in range(1, ENGINE_ATTEMPTS + 1):
        код, вывод = _launch(script, args, data_dir=data_dir, cwd=cwd, state=state)
        if код == 0:
            return код, вывод
        if RUNTIME_FATAL in вывод:
            повторим = RUNTIME_BEFORE_SCRIPT in вывод and попытка < ENGINE_ATTEMPTS
            _to_log(
                f"интерпретатор движка умер (попытка {попытка}"
                f"{', повторяем' if повторим else ', больше не повторяем'})",
                скрипт=script,
                каталог=data_dir,
            )
            if повторим:
...
def _launch(
    ...
    """Один запуск скрипта движка над указанным каталогом методики.
    ...
    if завершение.returncode < 0:
        ...
        # движок успевает напечатать начало вывода, и по одному коду возврата
...
def _made_from(store: Store, *, base_version: str, version: str) -> bool:
    """Записано ли в журнале, что ЭТА версия получилась из ЭТОЙ основы.

    Единственное, чем повтор отличается от возврата: имя каталога одинаково у
    обоих (оно считается по содержимому), а событие «из основы X получилась
    версия Y» есть только у настоящего повтора. Журнала может не быть вовсе
    (его стирали руками) — тогда ответ «нет», и вызывающий говорит то, что
    верно в обоих случаях, вместо того чтобы утверждать лишнее.
    """
    return any(
        событие.get("outcome") == "accepted"
        and событие.get("base_version") == base_version
        and событие.get("version") == version
        for событие in read_journal(store)
    )
...
def _occupied(store: Store, *, base_version: str, version: str) -> str:
    ...
    считается по содержимому (D050), поэтому одинаковое содержимое от одного
    набора и одной даты — одно имя каталога. Случая за этим два:

    * **повтор** — ту же правку от той же основы уже применяли, и результат
      лежит под этим именем; делать нечего, и так и сказано;
    * **возврат** — правка приводит методику к состоянию ДРУГОЙ версии, обычно
      предшественника основы: строку карты завели и тут же сняли, пункт
      выключили и включили обратно. Правка при этом НЕ применена, и одно слово
      на оба случая говорило агенту управляющей компании, что работа сделана,
      — хотя строка на месте (T218, issue #166).

    Возврат отказом остаётся: двух каталогов с одним содержимым, набором и
    датой в хранилище быть не может, а состояние, которого просят, в нём уже
    лежит готовой версией. Поэтому отказ называет оба выхода — опубликовать
    эту версию (это и есть откат, D049) или записать состояние под другим
    именем набора.
    """
