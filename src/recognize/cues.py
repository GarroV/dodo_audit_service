...
#: теряли силу: строка «Печь», расписанная на «Конвейерную ленту», «Подину» и
#: «Загрузочное окно», делала слово «печь» неразличающим, хотя все четыре
#: строки ведут в одну и ту же пару пунктов. Задача просила считать порог долей
...
def match_cues(note: str, cues: tuple[Cue, ...]) -> tuple[str, ...]:
    ...
    совпавшего в самой подсказке: строка «Печь» совпала целиком и стоит выше
    строки «Печь: экран настроек, время и температура выпекания», где совпало
    одно слово из шести.
    """
    words = stems(note)
    if not words:
