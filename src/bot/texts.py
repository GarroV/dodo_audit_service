...
TEXTS: dict[str, dict[str, str]] = {
    ...
    # в начале проверки — заранее, и когда бот ждёт кадр — по факту. Две копии
    # одной мысли разъезжаются молча: в одном месте поправили, в другом забыли,
    # и человек читает в одном продукте два разных правила. Собирает обе
    # `with_photo_rule`.
    #
    # Порядок «сначала фотография» из правила ушёл с T229 (D090): слова,
    # сказанные до кадра, теперь ждут его, а не получают отказ, — и требовать
    # порядка, которого продукт не требует, значило бы отговаривать человека от
    # того, что работает.
    "material.photo_required": {
        "ru": (
            "Фотофиксация обязательна: записи без кадра не бывает. "
            "Комментарий можно дать и до кадра, и после — подписью, отдельным "
            "сообщением или ответом на кадр; записью он станет, когда придёт фотография."
        ),
        "en": (
            "A photo is required: there is no record without one. "
            "The comment may come before or after the photo — as a caption, as a "
            "separate message, or as a reply to it; it becomes a record once the photo arrives."
        ),
    },
    # Слова пришли раньше кадра (T229, D090). Здесь стоял отказ «один
    # комментарий записью не станет»: он был правдой про запись и неправдой про
    # слова — те просто пропадали. Теперь это первая половина пары «принял /
    # сохранено»: бот говорит, что услышанное придержано и чего он ждёт, а
    # вторую половину скажет отбивка о сохранении, когда кадр придёт.
    "material.waiting_photo": {
        "ru": "Принял ваши слова и держу их. Жду фотографию — придёт кадр, соберу по ним запись.",
        "en": (
            "Got your words and I am holding them. Waiting for the photo — "
            "the record will be made when the frame arrives."
        ),
    },
    ...
    # Пачка кадров без комментария (T206, D081): разбор идёт по кадру, и
    # отбивка приходит списком отдельными сообщениями. Сколько кадров
    # разбирается, говорится один раз — до списка, а не по разу на кадр.
    "record.batch": {
        "ru": "Разбираю по кадру. Кадров: {count}, на каждый будет своё сообщение.",
        "en": "Analyzing frame by frame. Photos: {count}, each gets its own message.",
    },
    # Пачка больше предела (T232, решения D091 и D093). Про неразобранные кадры
    # бот ГОВОРИТ: молча отброшенные, они выглядели бы записанными, и аудитор ушёл
    # бы с точки, считая фотофиксацию сделанной. Сказано три вещи, и все три
    # нужны: сколько разобрано, что остальные НЕ разобраны, и что с ними делать.
    # Пачка больше пяти — исключение, а не рабочий случай (D093), поэтому предел
    # и не спрятан во «внутри системы обработаю порциями».
    "record.batch_over_limit": {
        "ru": (
            "Разбираю {limit} кадров из {count}: больше {limit} за раз не берусь. "
            "На каждый будет своё сообщение. Оставшиеся {rest} я не разбирал — "
            "пришлите их отдельной пачкой."
        ),
        "en": (
            "Analyzing {limit} of {count} photos: I take no more than {limit} at a time. "
            "Each gets its own message. The remaining {rest} are NOT analyzed — "
            "send them as a separate batch."
        ),
    },
    # Тот же вопрос, что и у одиночного кадра, но с номером кадра в пачке: без
    # номера аудитор не соотнесёт сообщения списка с тем, что снимал, а «Кадров:
    # 1» в каждом из пяти сообщений читалось бы как потеря остальных.
    "record.candidates_batch": {
        "ru": "Кадр {no} из {total}. Что записать?\n\n{lines}",
        "en": "Photo {no} of {total}. What should I record?\n\n{lines}",
    },
    # Кадр, присланный ответом на свои же слова, ушёл в ту запись (T205, D081).
    # Число кадров называется потому, что это единственное, что в записи
    # изменилось: пункт, класс и зона остались прежними, и «поправил» было бы
    # неправдой.
    "record.frame_attached": {
        "ru": "📎 Кадр добавлен к записи #{n}. Кадров в ней: {count}.",
        "en": "📎 Photo added to record #{n}. It now has {count} photo(s).",
    },
    # Движок кадр не принял. Молчать нельзя: аудитор считает, что фотофиксация
    # сделана, а её нет. Кадр при этом не потерян — он остался в списке
    # присланного и покажется при завершении (T068).
    "record.frame_failed": {
        "ru": (
            "Кадр к записи #{n} не прикрепился. Он не потерян — покажу его при завершении проверки."
        ),
        "en": (
            "The photo did not attach to record #{n}. It is not lost — I will list it "
            "when you finish."
        ),
    },
    ...
    # Отбивка о сохранении, ОДНА на все пути добавления записи (T230, решение
    # D090). Решение требует пары «принял / сохранено» не только для голосового,
    # а для каждого способа завести запись. Ключ поэтому один, и все показы
    # добавления подставляют его параметром `{stored}`: разные слова об одном и
    # том же событии читаются как разные события, и аудитор ищет в переписке
    # вторую запись там, где она одна.
    #
    # Правку записи (`record.corrected`) отбивка не покрывает намеренно: там
    # записи не прибавилось, и «сохранено» вместо «поправил» было бы той самой
    # подменой события, от которой этот ключ и заведён.
    #
    # Вычета в отбивке нет. Пример владельца в D090 назван с числом баллов, но
    # оценка по ходу обхода не показывается вовсе (T162, решение D072: «только в
    # конце»), а число здесь стоило бы подпроцесса движка на каждую запись.
    # Разойтись этим двум решениям нельзя молча — развилка вынесена диспетчеру.
    "record.stored": {
        "ru": "✅ Сохранено.",
        "en": "✅ Saved.",
    },
    ...
    # Строки карты здесь нет — у подтверждённой записи её не бывает. Подпись
    # текста та же, что на быстром пути (T219): текст записи там и там один и
    # тот же. Таблицы после каждого кадра нет (`docs/06-mvp-bot.md`).
    "record.confirmed": {
        "ru": "{stored}\n{line}{guess}\n{title}\n\nВ отчёт: «{note}»",
    ...
    # Текст записи подписан ТАК ЖЕ, как на пути с подтверждением (T219, #167).
    # Стояло «Ваши слова», и подпись была верной ровно наполовину: слова
    # действительно его, но это не эхо сказанного, а формулировка, которую
    # напечатает отчёт партнёру. Расписку в получении не вычитывают — документ
    # вычитывают, и дальше правка зоны кнопкой оставляла формулировку прежней:
    # партнёру уезжала запись, спорящая сама с собой, а увидеть это аудитору
    # было негде. Что слова его, сказано тут же в первой строке.
    "record.fixed": {
        "ru": (
            "{stored} Подтверждать не нужно — пункт нашёлся по вашим словам.\n\n"
            "{line}{guess}\n"
            "{title}\n\n"
            "В отчёт: «{note}»\n"
            "Строка карты: «{cue}»\n\n"
        ...
        "en": (
            "{stored} No confirmation needed — your words matched an item.\n\n"
            "{line}{guess}\n"
            "{title}\n\n"
            "Into the report: “{note}”\n"
            "Map line: “{cue}”\n\n"
...
def with_photo_rule(text: str, lang: str) -> str:
    ...
    проверки, её продолжение, ожидание кадра к сказанным словам — T229), и
    разъехаться им нечего стоит. Здесь же держится и отступ между сообщением и
    правилом — иначе в одном месте оно оказалось бы абзацем, а в другом хвостом
    строки.

    Правило звучит и ЗАРАНЕЕ, и по факту намеренно. Владелец: «надо
    чтобы человек понял что фото должно быть», — а понимает человек то, что
    прочитал до ошибки, а не только то, чем ему ответили потом.
    """
