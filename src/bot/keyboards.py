...
#: Предел подписи на кнопку, которая стоит в ряду ОДНА (T140, T217).
#:
...
ROW_TITLE_LIMIT = 2 * BUTTON_TITLE_LIMIT

#: Предел подписи ЗОНЫ — она стоит в ряду одна (T140, #111).
#:
#: Что ширина ряда даёт на боевой методике: десять зон, самое длинное название —
#: 36 знаков по-русски и 32 по-английски. При 34 резалось одно (по-русски) и
...
ZONE_TITLE_LIMIT = ROW_TITLE_LIMIT

#: Что стоит на кнопке ручного перечня перед формулировкой (T217, #150). Код и
#: разделитель, и считается это одной строкой: предел формулировки выводится из
#: неё вычитанием, чтобы длина кода не осталась записанной числом в двух местах.
...
def manual_keyboard(
    ...
    **Формулировке отведена ширина ряда за вычетом кода (T217, #150).** Пункты
    стоят по одному в ряд, а подпись резалась по `BUTTON_TITLE_LIMIT` — пределу,
    выведенному под кнопку в ПОЛОВИНУ ряда. Под формулировку оставалось 34
    знака на всю ширину экрана, и восемь из них забирал код: на боевой методике
    обрезанными оказывались 94 % пунктов при медианной длине формулировки в 64
    знака. Ручной перечень — последняя линия обороны, когда модель не помогла,
    и выбирать в нём приходится по обрубку.

    **Место возвращается шириной ряда, а не снятием кода.** Код на кнопке
    остаётся, и это не формальность: после обрезки до 60 знаков на боевой
    методике 11 пунктов из 123 совпадают подписью буква в букву — без кода
    аудитор выбирал бы из двух неразличимых кнопок. Он же связывает кнопку с
    методикой и отчётом, а связывать сущности формулировками нельзя.
    """
    builder = InlineKeyboardBuilder()
    for index, code, title in titles:
        prefix = f"{code}{MANUAL_CODE_SEPARATOR}"
        builder.button(
            text=prefix + _short(title, ROW_TITLE_LIMIT - len(prefix)),
            callback_data=f"{MANUAL_PICK_PREFIX}{index}",
        )
    builder.adjust(1)
