...
class Proposal:
    ...
    question: str = ""
    zone_hint: str = ""
    #: Зона названа в САМИХ этих словах, а не подставлена памятью о прошлой
    #: записи (T124, D047/D048). Отличать обязательно: запись по словам ложится
    #: без подтверждения, и выдать догадку за слова аудитора значит спрятать
    #: промах ровно там, где его некому заметить.
