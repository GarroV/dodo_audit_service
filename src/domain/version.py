...
#: Предел длины ИМЕНИ каталога в байтах. Это не политика продукта, а `NAME_MAX`
#: файловой системы (255 на APFS, ext4 и всём, на чём продукт работает): имя
#: длиннее каталогом не станет нигде, и отказать на нём словами лучше, чем
#: уронить `os.replace` голым `ENAMETOOLONG` посреди правки методики. Считается
#: в БАЙТАХ, а не в знаках, потому что предел байтовый: имя набора кириллицей
#: стоит по два байта на букву, и мерка в знаках отказала бы слишком поздно.
SEGMENT_BYTES = 255

#: Чего в имени каталога быть не может. Разделители обеих файловых систем и
#: нулевой байт: первый уводит чтение в чужой каталог, второй обрывает путь на
#: середине уже в системном вызове.
SEPARATORS = frozenset({"/", "\\", "\0"})


def is_one_segment(version: str) -> bool:
    """Годится ли идентификатор издания как ИМЯ КАТАЛОГА.

    Вопрос задают в двух местах: полка снимков домена ищет каталог издания
    идущей проверки (`domain.edition`), хранилище версий MCP — каталог издания
    записанной (`mcp.checklist._check_version`). Ответ обязан быть один, и
    сведён он сюда на дефекте T243: у домена стоял список запрещённого, у MCP —
    белый список латиницы, и издание с нелатинским именем набора полка
    принимала, а инструменты MCP не читали вовсе — ни письма, ни подписей полей.
    Ровно та же болезнь, что T236, только там расходились сверки содержимого, а
    здесь — сверки имени.

    **Список запрещённого, а не разрешённого.** Имя издания задаёт управляющая
    компания файлом `checklist_version.txt`, и оно бывает не латиницей: белый
    список отверг бы законные издания. Проверяется здесь ровно то, ради чего
    проверка вообще нужна, — что идентификатор останется ОДНИМ куском пути:
    он приезжает из файла проверки и из базы, а это данные, которые правят
    руками, и `../../` в них превратило бы поиск снимка в чтение чужого
    каталога.

    Длина при этом тоже часть вопроса «годится ли в имя каталога»: она мерится
    пределом файловой системы (`SEGMENT_BYTES`), а не выдуманным числом.
    """
    if not version or version in {".", ".."}:
        return False
    if SEPARATORS & set(version):
        return False
    return len(version.encode("utf-8")) <= SEGMENT_BYTES
