...
class Finding:
    ...
    #: Сырые слова аудитора об этом материале — то, что он сказал или написал
    #: ДО того, как из них получилась запись (T183). Текстом записи они не
    #: становятся намеренно: партнёру уходит формулировка по правилам фиксации,
    #: а сказанное на точке ему не предназначено. Хранятся ради двух вещей —
    #: померить признак вроде «это чистота» (T166) и разобрать, почему система
    #: промахнулась: предложение (D077) и запись у нас есть, а того, что сказал
    #: человек, не было нигде.
    #:
    #: Пусто — слов не было ВООБЩЕ (разбор голого кадра) либо запись начата до
    #: T183. Различаются эти два случая источником записи: у первого он `photo`.
    words: str = ""
    suggested_code: str = ""
    suggested_level: str = ""
    suggested_zone: str = ""
