...
def token_fingerprint(token: str) -> str:
    ...
    # Обещание докстроки, сделанное настоящим (`assert` тут нельзя: под `-O` он
    # исчезает вместе с проверкой). До неё `FINGERPRINT_LENGTH`
    # стояла константой, на которую никто не смотрел, и «проверяется здесь» было
    # неправдой — ошибка в этой функции доехала бы до отказа ограничения схемы,
    # то есть до места, где уже не видно, что пошло не так.
    if len(отпечаток) != FINGERPRINT_LENGTH:
        raise AccessError(
            f"отпечаток вышел длиной {len(отпечаток)} вместо {FINGERPRINT_LENGTH} — "
            f"схема такой не примет, и разбираться пришлось бы по отказу базы"
        )
    return отпечаток
