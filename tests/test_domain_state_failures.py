...
def test_старая_форма_записи_кадра_читается() -> None:
    ...
    assert f.suggested_code == "", (
        "у записи, заведённой до T181, предложения системы нет и выдумывать его нельзя: "
        "выданное за него совпадение выглядело бы в базе попаданием модели"
    )
    assert f.words == "", (
        "у записи, заведённой до T183, сырых слов аудитора нет; подставить сюда текст записи "
        "значило бы выдать формулировку по правилам фиксации за речь человека"
    )
