...
# --- T128: письмо идеальной проверки -----------------------------------------


def test_письмо_без_нарушений_не_пишет_про_отклонения(
    started: Callable[..., Run], report: Callable[..., Run]
) -> None:
    """Идеальная проверка — не повод писать партнёру про отклонения (T128).

    До правки письмо говорило «Что отметили: — незначительных отклонений D1: 0»
    и тут же обещало, что «перечисленные отклонения устраняются в рабочем
    порядке в сроки, указанные в отчёте». Партнёр получал письмо про
    нарушения, которых у него нет, и шёл искать их в отчёте.
    """
    r = report("letter")
    assert r.code == 0, r.text
    low = r.out.lower()
    assert "отклонений d1: 0" not in low, "ноль отклонений подан как находка"
    assert "перечисленные отклонения" not in low, "письмо ссылается на список, которого нет"
    assert "нарушений не зафиксировано" in low, "письмо не сказало главного: нарушений нет"


def test_английское_письмо_без_нарушений_не_пишет_про_отклонения(
    started: Callable[..., Run], report: Callable[..., Run]
) -> None:
    """То же по-английски: демо и партнёры за границей читают именно его."""
    r = report("letter", "--lang", "en")
    assert r.code == 0, r.text
    low = r.out.lower()
    assert "deviations: 0" not in low, "ноль отклонений подан как находка"
    assert "deviations listed above" not in low, "письмо ссылается на список, которого нет"
    assert "no violations were recorded" in low, "письмо не сказало главного: нарушений нет"
