...
def test_туннель_живёт_в_сетевом_пространстве_сервера() -> None:
    """Туннель ходит к ПЕТЛЕ сервера, а не к нему по имени в сети стенда.

    Это единственная строка, которой держится вся защита доступа снаружи
    (T256). Уберите её — и туннель начнёт ходить к `mcp:8265`, то есть сервер
    станет доступен по сети compose: любому соседнему сервису, а на общей
    площадке это и есть публикация проверок партнёров наружу. Сам сервер такую
    настройку не отвергнет: для него запрос придёт снаружи, а не с петли, и
    отказывать он умеет только своему адресу прослушивания.

    Проверяется составом файла, а не запуском: поднимать здесь нечего, а
    строка обязана пережить любую правку соседних сервисов.
    """
    # Ищется НАСТРОЙКА, а не упоминание: та же строка стоит в комментарии рядом,
    # и поиск по подстроке был бы зелёным даже со снятой настройкой. Проверено
    # порчей: первая редакция этого теста ровно так и молчала.
    строки = COMPOSE_FILE.read_text(encoding="utf-8").splitlines()
    assert any(с.strip() == 'network_mode: "service:mcp"' for с in строки), (
        "у сервиса туннеля снят network_mode на сервер: туннель пойдёт к нему по сети "
        "стенда, и сервер окажется доступен соседям по общей площадке"
    )
