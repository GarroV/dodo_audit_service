...
async def test_отказ_сборки_не_показывает_внутренностей(
    domain_env: Path, стенд_без_рендерера: None, caplog: pytest.LogCaptureFixture
) -> None:
...
async def test_служебный_префикс_не_удваивается(
    domain_env: Path, стенд_без_рендерера: None
) -> None:
...
async def test_отказ_сборки_говорит_что_делать(domain_env: Path, стенд_без_рендерера: None) -> None:
    ...
    知 — иначе он решит, что день пропал, и начнёт проверку заново.
    """
    начать_с_записью()
    bot, session = make_bot()
