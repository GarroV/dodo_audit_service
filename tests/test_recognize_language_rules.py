...
`RULES = load_rules()`): если что-то из двадцати проверок ниже перестанет
...
поле и позитивный сценарий третьего языка). Здесь — оставшиеся девятнадцать
проверок на границе `load_rules`/`_one`/`_words` плюс двадцатая, отдельная:
...
from src.recognize.language import COLUMN_WORDS, THRESHOLDS, fleeting, load_rules, section_headings
...
def test_беглая_гласная_строкой_вместо_списка_это_отказ(tmp_path: Path) -> None:
    """Беглая гласная — список пар, а не одна строка.

    Строка «ek» молча разобралась бы как список однобуквенных пар («e», «k»),
    и правило снимало бы по одной букве там, где на самом деле пары нет вовсе —
    молчаливая порча основы у каждого слова языка.
    """
    язык = copy.deepcopy(ВАЛИДНЫЙ_ЯЗЫК)
    язык["fleeting"] = "ek"
    файл = _записать(tmp_path, язык)
    ...
    assert "xx/fleeting" in str(отказ.value), str(отказ.value)


def test_беглая_гласная_не_парой_букв_это_отказ(tmp_path: Path) -> None:
    """Беглая гласная обязана быть РОВНО парой букв: правило снимает первую.

    Тройка вроде «ок?» означала бы уже не беглую гласную, а произвольную резку
    основы, — молчаливое принятие такой записи испортило бы разбор каждого
    слова, оканчивающегося на эти буквы.
    """
    язык = copy.deepcopy(ВАЛИДНЫЙ_ЯЗЫК)
    язык["fleeting"] = ["ок?"]
    файл = _записать(tmp_path, язык)
    ...
    assert "парой" in str(отказ.value), str(отказ.value)


def test_пустой_список_беглых_гласных_это_не_отказ(tmp_path: Path) -> None:
    """Пустой список — законное «в этом языке беглых гласных нет», не отказ.

    Это позитивный случай, и он важнее прочих: именно он отличает «в этом
    языке беглых гласных нет» (законно, как у английского) от «поле забыли
    объявить» (отказ, случай выше). Если бы проверка отклоняла пустой список
    наравне с непустыми, английский язык продукта не смог бы объявить своё
    правило вовсе.
    """
    язык = copy.deepcopy(ВАЛИДНЫЙ_ЯЗЫК)
    язык["fleeting"] = []
    ...
    assert fleeting(правила) == ()


def test_беглая_гласная_в_верхнем_регистре_это_отказ(tmp_path: Path) -> None:
    """Сравнение слов идёт с уже понижённым текстом — как у стоп-слов и окончаний.

    Беглая гласная «ОК» не встретилась бы ни разу в приведённом к нижнему
    регистру слове: проверка идёт общей функцией `_words`, как и у прочих
    списков слов языка, поэтому отказ обязан сработать так же.
    """
    язык = copy.deepcopy(ВАЛИДНЫЙ_ЯЗЫК)
    язык["fleeting"] = ["ОК"]
    файл = _записать(tmp_path, язык)
