...
**Что в этом файле изменила задача T201 (решение владельца D080).** Тогда
считалось, что продолжить сданную проверку — законный ход: «аудитор мог
вспомнить ещё одно нарушение». Владелец это отменил — «отчеты мы не правим», —
и два случая здесь переписаны: кнопки «Продолжить» на сданной проверке больше
нет, а число записей запечатанность не отменяет. Сам запрет и все его входы
стережёт `tests/test_bot_sealed.py`; здесь остался признак сдачи как таковой.
"""
...
@pytest.mark.asyncio
async def test_за_аудитора_бот_ничего_не_решает(domain_env: Path) -> None:
    """Сданная — не повод действовать самому: показаны двери, нажимает человек.

    Дверей с T201 две, а не три: дописать в сданную проверку нельзя (D080,
    `tests/test_bot_sealed.py`). Но ни одну из оставшихся бот не открывает сам —
    ни новую проверку не заводит, ни сданную не убирает.
    """
    начать()
    записать()
    bot, session = make_bot()
    ...
    assert session.keyboard_data() == ["start:resume:new", "start:sealed:drop"]
    state = get_state(CHAT_ID)
    assert state is not None and len(state.findings) == 1, "бот распорядился проверкой сам"
...
@pytest.mark.asyncio
async def test_число_записей_запечатанность_не_отменяет(domain_env: Path) -> None:
    """Обратная сторона решения D080, и это ПЕРЕПИСАННЫЙ случай.

    До T201 признак сдачи сверялся с числом записей: дописанная после сдачи
    запись означала устаревший отчёт, и проверка снова звалась незавершённой.
    Дописывать теперь нельзя ничем, а сверка с числом стала бы дырой в запрете —
    проверка, в которой записей всё-таки стало больше (заметки от прежней версии
    бота, правка файла руками), снова открылась бы для дописывания.

    Запись здесь делается движком в обход бота намеренно: через бота она уже не
    проходит, а проверяется именно то, что признак не зависит от их числа.
    """
    начать()
    записать()
    bot, session = make_bot()
    ...
    assert "уже сдана" in session.last_text, "лишняя запись расколдовала сданную проверку"
    assert "start:resume:continue" not in session.keyboard_data()
