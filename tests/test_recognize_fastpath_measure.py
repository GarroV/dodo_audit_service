...
в ней дважды («Мебель и мебель участка»), даёт ровно такой легитимный пропуск
на одном из двух вхождений и честный отказ на другом, и тест ловит оба исхода
поимённо, а не долей.
"""
...
def карта_с_повтором_основы(
    ...
    Это ровно тот легитимный случай пропуска, который объясняет
    `render_negation_section` (T195, задача #160): отрицание ОДНОГО вхождения
    основы не снимает срабатывания, потому что второе вхождение той же
    основы остаётся сказанным утвердительно, и покрытие строки карты
    (`_covered`, `src/recognize/fastpath.py`) всё ещё выполняется. Дописываем
    в копию: боевую карту сюда тянуть нельзя (D002), а без такой строки замер
    отрицания проверял бы только «хороший» случай и не показал бы разницу
    между честным пропуском и дырой в правиле.
    """
...
def test_пропуск_это_срабатывание_на_отрицании_а_не_другой_код(
    карта_с_повтором_основы: Path,
) -> None:
    """T195: повтор основы внутри строки — легитимный пропуск, не дефект правила.

    Строка «Мебель и мебель участка» несёт основу «мебель» дважды. Отрицание
    ПЕРВОГО вхождения не снимает срабатывания: второе вхождение той же основы
    остаётся сказанным утвердительно, и `fast_path` срабатывает как раньше —
    это и есть пропуск, который считается «правило не заметило», а не
    «сработал другой код» (кода тут ровно один, CLN06, что и проверяется).
    Отрицание уникального слова строки («участка»), наоборот, обязано снять
    срабатывание — это нормальная работа T195, а не тот же самый пропуск.
    """
    ...
    assert повтор_основы.missed is True, "повтор основы обязан оставаться легитимным пропуском"
...
def test_код_возврата_не_зависит_от_раздела_про_отрицание(
    ...
    Карта фикстуры нарочно несёт легитимный пропуск (T195): если бы код
    возврата зависел от раздела 2, эта же карта красила бы прогон, в котором
    первый раздел ни разу не ошибся. Проверяем все три исхода первого раздела
    (0, 1, 2 — старое правило) на одной и той же «грязной» карте, чтобы
    исключить именно эту зависимость, а не что-то соседнее.
    """
