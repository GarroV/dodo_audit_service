...
def test_содержимое_проверки_остаётся_нетронутым(сервер: str) -> None:
    """Записи в СОДЕРЖИМОЕ проверки нет ни у одного инструмента.
    ...
    проверка заводится новой. Поэтому исключений у правила два, оба названы
    видом инструмента, и оба пересчитываются: методика — своим модулем и своим
    хранилищем, снятие — своим модулем и ровно одним инструментом.

    Проверяется не по списку имён, а по виду: у всего, что не объявлено
    методикой или снятием, обработчик обязан лежать в модуле чтения проверок,
    где записи нет вовсе.
    """
    ответ = _rpc(сервер, "tools/list")
    объявленные = {инструмент["name"] for инструмент in ответ["result"]["tools"]}

    снимающие = [spec for spec in TOOLS if spec.kind == KIND_RETRACTION]
    assert len(снимающие) == 1, (
        "снятие проверки — названное исключение из правила, и оно обязано быть "
        "одним: второй инструмент этого вида означал бы, что исключение "
        "перестало быть исключением, а заслон ниже его пропустит"
    )
    assert снимающие[0].name == "retract_inspection"
    assert снимающие[0].handler.__module__ == "src.mcp.retraction", (
        "обработчик снятия обязан лежать в своём модуле: в модуле чтения ему не "
        "место, а в модуле методики он получил бы права методики"
    )
