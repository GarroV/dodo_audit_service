...
(`ChecklistVersionMismatch`), а не тихий пересчёт. Пересчитать по прежней
методике сегодня физически нечем — старых данных нигде нет, хранилище версий
только строится, — а посчитать по новой под старой отметкой запрещает D033.
Выход из тупика есть, но только явный: `sync_checklist_version` переставляет
отметку и оставляет след в самой проверке.
