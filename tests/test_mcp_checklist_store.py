...
def test_негодное_имя_набора_это_отказ(хранилище: Store, имя: str, про_что: str) -> None:
    ...
    имени ВЕРСИИ, — и знак пути он поймал бы тоже, но сказал бы про версию, а
    человек пошёл бы искать несуществующую версию вместо того, чтобы
    переименовать набор.

    Кириллицу второй сторож с T243 не ловит вовсе, и здесь она отвергается
    одним этим правилом — не по алфавиту, а потому что это единственное место,
    где имя набора ПРИДУМЫВАЮТ. Имя уже изданного набора приходит от
    управляющей компании файлом `checklist_version.txt`, наследуется как есть
    и латиницей быть не обязано: тест на это — в
    `tests/test_mcp_edition_name.py`.
    """
    with pytest.raises(ChecklistError) as отказ:
...
@pytest.mark.parametrize("версия", ["../побег", "imf/вложенно", "имф\\побег", ".", "..", "", " "])
def test_версия_с_знаком_пути_это_отказ(хранилище: Store, версия: str) -> None:
    ...
    Кириллицы среди негодных имён БОЛЬШЕ НЕТ, и это не послабление (T243). Имя
    набора задаёт управляющая компания, оно бывает не латиницей, и сторож здесь
    охраняет путь, а не алфавит: издание `имф-2026-09-01-…` законно, а
    инструменты MCP его не читали вовсе. Что кириллическое имя доходит до
    второго сторожа, а не отвергается первым, проверяет тест ниже.
    """
    with pytest.raises(ChecklistError) as отказ:
        read_items(хранилище, version=версия)

    assert "именем каталога" in str(отказ.value)


def test_кириллическое_имя_версии_доходит_до_второго_сторожа(хранилище: Store) -> None:
    """T243: сторож имени охраняет путь, а не алфавит.

    Такой версии в хранилище нет, и отказ обязан быть про ЭТО — «такой версии
    в хранилище нет», а не «не похоже на версию методики». Второй отказ послал
    бы человека переименовывать законное издание его же управляющей компании.
    """
    with pytest.raises(ChecklistError) as отказ:
        read_items(хранилище, version="имф-2026-09-01-3f5a91b2c7d0")

    сказано = str(отказ.value)
    assert "в хранилище нет" in сказано
    assert "именем каталога" not in сказано
