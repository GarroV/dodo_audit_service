...
def test_подсказка_срабатывает_на_словах_а_не_на_точном_совпадении(domain_env: Path) -> None:
    ...
    # «маркировки», «Огнетушитель», «грызуны», «Печь».
    assert "PRD09" in match_cues("тара с неверной маркировкой", подсказки)
    assert "SFT08" in match_cues("огнетушители просрочены", подсказки)
    assert "FSB17" in match_cues("следы грызунов у мешков", подсказки)
...
def test_раздел_порогов_не_становится_подсказкой(tmp_path: Path) -> None:
    ...
    Строка «PRD09 нет маркировки → менее 4 упаковок» отвечает на вопрос «какой
    класс», а не «что видно». Попав в подсказки, она подняла бы PRD09 на любое
    слово «упаковка» и перемешала бы порядок кандидатов.
    """
    ...
    карта.write_text(
        ...
        "| Пункт | D1 | D2 |\n"
        "|---|---|---|\n"
        "| PRD09 нет маркировки | менее 4 упаковок | 4–5 продуктов, см. PRD11 |\n\n"
        "## Чистота\n\n"
        "| Объект на фото | Грязь | Поломка |\n"
        "|---|---|---|\n"
        "| Печь | CLN05 | TEH05 |\n",
        encoding="utf-8",
    )
