...
def test_служебные_пункты_не_предлагаются(domain_env: Path) -> None:
    ...
    итог = shortlist("температура в холодильнике +9", zone_hint="fridge", chat_id=NO_CHAT)
...
def test_пункты_ручного_решения_аудитора_не_предлагаются(domain_env: Path) -> None:
    итог = shortlist("массовые мелкие нарушения по всей точке", zone_hint="dining", chat_id=NO_CHAT)
...
def test_карта_кадров_поднимает_свои_коды_в_начало(domain_env: Path) -> None:
    итог = shortlist("печь грязная, под лентой нагар", zone_hint="hot_kitchen", chat_id=NO_CHAT)
...
def test_карта_кадров_добавляет_код_которого_нет_в_зоне(domain_env: Path) -> None:
    ...
    итог = shortlist("печь в нагаре", zone_hint="dining", chat_id=NO_CHAT)
...
def test_без_подсказки_зоны_берутся_все_нарушения(domain_env: Path) -> None:
    ...
    итог = shortlist("грязно", zone_hint=None, chat_id=NO_CHAT)
...
def test_неизвестная_зона_отвергается(domain_env: Path) -> None:
    with pytest.raises(ValidationError):
        shortlist("грязно", zone_hint="кухня", chat_id=NO_CHAT)
...
def test_без_карты_кадров_перечень_не_режется_и_отказа_нет(
    ...
    """
    с_картой = shortlist("печь в нагаре", zone_hint="hot_kitchen", chat_id=NO_CHAT)

    (data_copy / "photo-cues.md").unlink()
    monkeypatch.setenv("AUDIT_DATA_DIR", str(data_copy))
    with caplog.at_level(logging.WARNING):
        без_карты = shortlist("печь в нагаре", zone_hint="hot_kitchen", chat_id=NO_CHAT)
...
def test_подсказка_срабатывает_на_словах_а_не_на_точном_совпадении(domain_env: Path) -> None:
    подсказки = load_cues(chat_id=NO_CHAT)
...
def test_информационные_пункты_остаются_в_перечне(domain_env: Path) -> None:
    ...
    """
    итог = shortlist("на дисплее холодильной камеры +6", zone_hint="fridge", chat_id=NO_CHAT)
...
def test_раздел_порогов_не_становится_подсказкой(tmp_path: Path) -> None:
    ...
    подсказки = load_cues(карта, chat_id=NO_CHAT)
    ...
    assert class_thresholds(карта, chat_id=NO_CHAT).splitlines()[-1].startswith("| PRD09")
