"""Дорогие вызовы не должны идти в цикле событий бота (T101, T107, T117).
...
Подпроцесс — не единственный способ занять цикл: `fast_path` (T117) ни в какую
сеть не ходит и ничего не запускает, но читает с диска методику и карту кадров,
и это 2.3 мс на сработавшем комментарии против 0.046 мс у `get_state`. Граница
проходит не по «движок или не движок», а по цене: то, что стоит миллисекунды,
уезжает в поток.
"""
...
#: Вызовы, которые стоят миллисекунды и потому обязаны идти в поток. Замерено:
#: add_finding 27 мс, attach_photo 24 мс, score 26 мс (T101); fast_path 2.3 мс
#: на сработавшем комментарии и 1.2 мс на отказе (T117) — он читает с диска
#: методику и карту кадров при каждом вызове, кеша там нет намеренно.
BLOCKING = frozenset(
    {
        "add_finding",
        "attach_photo",
