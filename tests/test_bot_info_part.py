...
кнопкой и уезжает в отчёт на языке ОТЧЁТА, подпись к кадру становится ответом.
Сам кадр с T179 уходит в отчёт рядом с ответом, и проверяется это отдельным
набором — `tests/test_bot_info_photo.py`.
...
async def test_подпись_к_кадру_остаётся_ответом_на_вопрос(domain_env: Path) -> None:
    """Кадр приложением быть стал (T179), а подпись ответом быть не перестала.

    Про сам кадр — `tests/test_bot_info_photo.py`: там он проверяется до
    страницы PDF. Здесь защищается только то, что подпись не потерялась по
    дороге, — это утверждение принадлежит информационной части, а не кадру.
    """
    начать()
    bot, _session = make_bot()
    dp = build_dispatcher(SETTINGS)
