...
def test_чужой_сервер_за_туннелем_доступом_не_считается(чужой_за_туннелем: str) -> None:
    ...
    Ловится это НЕ заголовком `Server`: посредник подменяет его своим именем, и
    у настоящего Cloudflare там стоит «cloudflare» — проверка по заголовку
    краснела бы в рабочей настройке и молчала бы в подложной, где заголовок
    подделать ничего не стоит. Чужого ловит последний шаг, по `serverInfo`.
    """
