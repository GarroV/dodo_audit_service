...
select
    %(tenant)s, %(unit_id)s, 1, 'Плановая', current_date, 'ru',
...
def test_выборка_идёт_по_индексу_а_не_полным_проходом(db_env: str) -> None:
    ...
    """
    _насыпать(db_env, арендатор=АРЕНДАТОР_А, сколько=8000)
    _насыпать(db_env, арендатор=АРЕНДАТОР_Б, сколько=2000)
