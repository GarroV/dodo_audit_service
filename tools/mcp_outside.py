...
3. **Без токена — отказ 401.** Доказывает, что за туннелем дверь заперта, а не
   открыта настежь. Кто именно за ней стоит, этот шаг НЕ доказывает: посредник
   подменяет заголовок `Server` своим именем (Cloudflare так и делает), и
   требовать здесь наше имя значило бы краснеть в рабочей настройке.
...
def reachability_results(url: str, token: str) -> list[Result]:
    ...
    # Заголовок `Server` здесь — подсказка, а не приговор, и это проверено на
    # живом туннеле: Cloudflare ПОДМЕНЯЕТ его своим именем, поэтому проверка,
    # требовавшая нашего имени, не могла пройти через туннель никогда — то есть
    # краснела ровно в рабочей настройке. Доказывает «за туннелем наш сервер»
    # шаг 5: `serverInfo` в ответе на `initialize`, а его подменить по дороге
    # нельзя, не разобрав тело JSON-RPC.
    результаты.append(
        Result(
            "за туннелем отвечает сервер",
            OK,
            f"Server = {представился!r}" if представился else "заголовок Server не назван",
        )
    )
