...
# PYTHONDONTWRITEBYTECODE не для чистоты — он делает честной проверку порчей.
# Python инвалидирует кэш байткода по паре «время правки в СЕКУНДАХ + размер».
# Порча, не меняющая размер и живущая меньше секунды, — а это ровно порча вида
# `if x < 0:` → `if False:` — подбирает байткод предыдущего прогона, и тест
# проходит на сломанном коде. Замерено 03.09 на этом проекте: 12 промахов из
# 12 при том же размере, 0 из 8 при изменившемся и 0 из 8 с этим ключом.
# Опаснее обратный случай: восстановление после порчи тоже берёт кэш, и тогда
# «тест поймал дефект» — ложь, а тест объявлен проверенным.
test:
	PYTHONDONTWRITEBYTECODE=1 $(VENV)/pytest
